...
public class MonsterFinal extends JefeFinal {

		protected static final double FIRING_FREQUENCY2 = 0.005;
		public MonsterFinal(Stage stage) {
			super(stage);
			setSpriteNames( new String[] {"Jefes/pajarraco.gif"});
			setFrameSpeed (35);
			this.vida=5000;
			this.vidaT=5000;
			this.puntosAObtener=200000;
		...
		public void fire() {
			...
			m.setX(x+getWidth()-90);
			m.setY(y + getHeight());
			stage.addActor(m);
			stage.getSoundCache().playSound("sonidos/photon.wav");
			LaserBola n = new LaserBola(stage);
			n.setX(x+getWidth()-230);
			n.setY(y + getHeight());
			stage.addActor(n);
			stage.getSoundCache().playSound("sonidos/photon.wav");
