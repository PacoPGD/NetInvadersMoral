		...
		while (isVisible() && !gameEnded) {
			...
			if(fichaTiempo < (int)(eventTime/100)){
				...
				if(contador1==0){
					...
					Charla.getCharla().setMensaje("Vaya, esperad un momento, noto en mis sensores que hay un planeta cercano con una extraña masa en su interior");
				}
				if(contador1==50){
				...
				if(contador1==100){
					...
					Charla.getCharla().setMensaje("Es cierto, la galaxia necesita a un heroe que los salve del mal, nos desviaremos y destruiremos esa masa extraña");
				}
				if(contador1==150){
					Charla.getCharla().pdamonConf();
					Charla.getCharla().setMensaje("No es tan facil como parece, si entrais al planeta podeis sufrir desviaciones en el rumbo, que serán caoticas, la gravedad extrema puede que os impida salir del mismo");
				}
				if(contador1==220){
					Charla.getCharla().defaultConf();
					Charla.getCharla().setMensaje("Bien, está decidido, asumiremos riesgos");
				}
				if(contador1==270){
