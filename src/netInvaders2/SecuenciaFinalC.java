	...
	public void jugar() {
		...
		while (isVisible() && !gameEnded) {
			...
			if(fichaTiempo < (int)(eventTime/100)){
				...
				if(contador1==150){
					Charla.getCharla().TelecoConf();
					Charla.getCharla().setMensaje("Pero no importa... tengo mas compañeros en mari*a d´or ciudad de va**c*o*es");
				}
				if(contador1==200){
				...
				if(contador1==350){
					Charla.getCharla().defaultConf();
					Charla.getCharla().setMensaje("Señores... buen trabajo");
				}
				if(contador1==370){
					Charla.getCharla().TelecoConf();
					Charla.getCharla().setMensaje("ESCUCHADME COÑOOOOOOOOOO");
				}
				if(contador1==400){
